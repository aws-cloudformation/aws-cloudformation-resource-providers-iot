...
public class ListHandlerTest extends HandlerTestBase {
    ...
    public void handleRequest_SimpleSuccess() {
        ...
        when(proxy.injectCredentialsAndInvokeV2(eq(LIST_DEFAULT_REQUEST), any()))
    ...
    @Test
    public void handleRequest_With_DEFAULT_TYPE_TARGET() {
        ResourceModel model = createDefaultModelWithTargetId();

        ResourceHandlerRequest<ResourceModel> request = createCfnRequest(model);

        when(proxy.injectCredentialsAndInvokeV2(eq(LIST_DEFAULT_REQUEST), any()))
                .thenReturn(LIST_RESPONSE_WITH_DEFAULT_TYPE);
