...
public class DeleteHandler extends BaseHandler<CallbackContext> {
    ...
    public ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        //delete resource
        DeleteV2LoggingLevelRequest deleteV2LoggingLevelRequest = DeleteV2LoggingLevelRequest.builder()
                .targetType(targetType)
                .targetName(targetName)
                .build();

        try {
            proxy.injectCredentialsAndInvokeV2(deleteV2LoggingLevelRequest, iotClient::deleteV2LoggingLevel);

        } catch (RuntimeException e) {
            return ExceptionTranslator.translateExceptionToProgressEvent(model, e, logger);
        }

        logger.log(String.format("Delete %s [%s] successfully", ResourceModel.TYPE_NAME, model.getTargetId()));
