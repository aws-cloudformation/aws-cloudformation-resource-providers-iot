...
public class DeleteHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        if (StringUtils.isEmpty(model.getRuleName())) {
            throw new CfnNotFoundException(InvalidParameterValueException.builder().message("Parameter 'RuleName' must be provided.").build());
        }
    ...
    private DeleteTopicRuleResponse deleteResource(final DeleteTopicRuleRequest deleteRequest, final ProxyClient<IotClient> proxyClient) {
        try {
            tryGetTopicRule(deleteRequest.ruleName(), proxyClient);
