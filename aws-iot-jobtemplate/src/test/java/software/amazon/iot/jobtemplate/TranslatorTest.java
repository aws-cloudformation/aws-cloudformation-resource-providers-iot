...
class TranslatorTest {
    ...
    @Test
    void getAbortConfigModelToSDKMultipleCriteria() {
        List<AbortCriteria> criteria = new ArrayList<>();
        for(int i = 0; i < 3; i++) {
            criteria.add(AbortCriteria.builder()
                    .action(ACTION)
                    .failureType(FAILURE_TYPE)
                    .minNumberOfExecutedThings(minNumberOfExecutedThings)
                    .thresholdPercentage(thresholdPercentage)
        ...
        assertNotNull(sdkAbortConfig);
        assertEquals(criteria.size(), sdkAbortConfig.criteriaList().size());
        for(software.amazon.awssdk.services.iot.model.AbortCriteria translatedCriteria : sdkAbortConfig.criteriaList()) {
            assertEquals(ACTION, translatedCriteria.action().toString());
            assertEquals(FAILURE_TYPE, translatedCriteria.failureTypeAsString());
            assertEquals(minNumberOfExecutedThings, translatedCriteria.minNumberOfExecutedThings());
            assertEquals(thresholdPercentage, translatedCriteria.thresholdPercentage());
        }
    }
    ...
    @Test
    void GetAbortConfigSDKtoModelMultipleCriteria() {
        List<software.amazon.awssdk.services.iot.model.AbortCriteria> criteriaList = new ArrayList<>();
        for(int i = 0; i < 3; i++) {
            criteriaList.add(software.amazon.awssdk.services.iot.model.AbortCriteria.builder()
                    .action(ACTION)
                    .failureType(FAILURE_TYPE)
                    .minNumberOfExecutedThings(minNumberOfExecutedThings)
                    .thresholdPercentage(thresholdPercentage)
        ...
        assertNotNull(modelAbortConfig);
        assertEquals(criteriaList.size(), modelAbortConfig.getCriteriaList().size());

        for(AbortCriteria abortCriteria : modelAbortConfig.getCriteriaList()) {
            assertEquals(ACTION, abortCriteria.getAction());
            assertEquals(FAILURE_TYPE, abortCriteria.getFailureType());
            assertEquals(minNumberOfExecutedThings, abortCriteria.getMinNumberOfExecutedThings());
            assertEquals(thresholdPercentage, abortCriteria.getThresholdPercentage());
